...
def process(message):
    ...
    try:
        ...
        with requests_cache.enabled('movie_cache', backend='sqlite', expire_after=86400):
            ...
            r = requests.get('https://api.themoviedb.org/3/search/movie?api_key='+TMDB_API_KEY+'&query='+ str(message))
        ...
        template = TextTemplate('Title: ' + data['title'] +
                                ...
                                '\nOverview: ' + data['overview'] + '\nIMDb Link', 'https://www.imdb.com/title/' + data['imdb_id'] + '/').get_message()
